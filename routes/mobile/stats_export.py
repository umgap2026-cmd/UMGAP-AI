...
def _salary_exprs(cur):
    """Kembalikan ekspresi SQL gaji yang aman sesuai kolom yg ada di DB."""
    ...
    if has_ps:
        daily_e   = "COALESCE(ps.daily_salary, 0)"
        monthly_e = "COALESCE(ps.monthly_salary, 0)" if has_monthly else "0"
        saltype_e = "COALESCE(ps.salary_type, 'daily')" if has_saltype else "'daily'"
        join_e    = "LEFT JOIN payroll_settings ps ON ps.user_id = u.id"
        group_e   = (", ps.daily_salary"
    ...
    return daily_e, monthly_e, saltype_e, join_e, group_e


def _fetch_detail(s, e):
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    try:
        daily_e, monthly_e, saltype_e, join_e, _ = _salary_exprs(cur)
        cur.execute(f"""
            SELECT u.name                AS employee_name,
                   {daily_e}            AS daily_salary,
                   {monthly_e}          AS monthly_salary,
                   {saltype_e}          AS salary_type,
...
def _fetch_summary(s, e):
    conn = get_conn(); cur = conn.cursor(cursor_factory=RealDictCursor)
    try:
        daily_e, monthly_e, saltype_e, join_e, group_e = _salary_exprs(cur)
        cur.execute(f"""
            ...
            LEFT JOIN attendance a ON a.user_id = u.id
                AND a.work_date >= %s AND a.work_date < %s
            WHERE u.role = 'employee'
            GROUP BY u.id, u.name{group_e}
            ORDER BY u.name ASC;
        """, (s, e))
        return [dict(r) for r in cur.fetchall()]
    except Exception as ex:
        import traceback
        print(f"[stats_export._fetch_summary] ERROR: {ex}\n{traceback.format_exc()}")
        raise
    finally:
        cur.close(); conn.close()
...
def _build(detail, summary, label):
    ...
    last1 = DR + len(detail) - 1
    tr    = last1 + 2 if detail else 7
    ...
    for col in [9, 10, 11]:
        ...
        ce = ws1.cell(row=tr, column=col,
                      value=f"=SUM({cl}{DR}:{cl}{last1})" if detail else 0)
    ...
    last2 = SR + len(summary) - 1
    gr    = last2 + 2 if summary else 7
