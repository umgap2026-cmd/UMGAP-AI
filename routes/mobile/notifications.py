# Fix: ganti kolom 'message' → 'body' sesuai schema announcements yang benar
...
def mobile_notifications():
    ...
    try:
        cur.execute("""
            SELECT
                a.id, a.title, a.body AS message, a.created_at,
        ...
        return mobile_api_response(ok=True, message="OK",
            data={"notifications": items}, status_code=200)
    finally:
...
def mobile_notifications_read(ann_id):
    ...
    try:
        ...
        return mobile_api_response(ok=True,
            message="Notifikasi ditandai sudah dibaca.", data={}, status_code=200)
    finally:
        cur.close(); conn.close()
