...
def mobile_login_google():
    ...
    try:
        ...
        # Verifikasi audience — WAJIB cocok dengan salah satu Client ID kita,
        # supaya id_token yang diterbitkan untuk aplikasi lain tidak bisa dipakai login ke UMGAP.
        ...
        allowed = [cid for cid in (web_client_id, android_client_id) if cid]
        if not allowed or not token_aud or token_aud not in allowed:
            print(f"[Google] Ditolak: aud={token_aud} tidak cocok dengan client ID terdaftar")
            return mobile_api_response(
                ok=False, message="Token Google tidak valid untuk aplikasi ini.",
                status_code=401)
